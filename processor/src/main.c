...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "subtaxh")) != -1)
    {
        switch (opt)
        {
            ...
            case 'h':
                ...
                printf("  -a : Launch the program 50 times to create an animation of bridge's deformation (in post-processor)\n");
                printf("  -x : Launch the program 50 times to create an animation of car's movement (in post-processor)\n");
    ...
    if      (bridgeSimplified_Usage) { meshPath = "../../reports/data/mesh_simplified.txt"; problemPath = "../../reports/data/problem_simplified.txt";  solutionPath = "../../reports/data/UV_simplified.txt"; }
    else if (exampleBeam_Usage)      { meshPath = "../../reports/data/mesh_beam.txt";       problemPath = "../../reports/data/problem_beam.txt";        solutionPath = "../../reports/data/UV_beam.txt"; }
    else if (exampleUForm_Usage)     { meshPath = "../../reports/data/mesh_example.txt";    problemPath = "../../reports/data/problem_example.txt";     solutionPath = "../../reports/data/UV_example.txt"; }
    else                             { meshPath = "../../reports/data/mesh.txt"; problemPath = "../../reports/data/problem.txt"; solutionPath = "../../reports/data/UV.txt"; }
    ...
    else
    {
        ...
        if (access("../data/animations", F_OK) == -1) { system("mkdir -p ../../reports/data/animations"); }
        ...
        for (int i = 1; i <= NB_IMAGES_ANIMATION; i++)
        {
            ...
            if (animationPosition) { sprintf(filename, "../../reports/data/animations/UV_animation_position_%d.txt", i); }
            else
            {
                if      (exampleUForm_Usage)     { sprintf(filename, "../../reports/data/animations/UV_example_%d.txt", i); }
                else if (exampleBeam_Usage)      { sprintf(filename, "../../reports/data/animations/UV_beam_%d.txt", i); }
                else if (bridgeSimplified_Usage) { sprintf(filename, "../../reports/data/animations/UV_simplified_%d.txt", i); }
                else                             { sprintf(filename, "../../reports/data/animations/UV_%d.txt", i); }
            }
